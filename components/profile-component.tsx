...
import { AuthSnapshot } from "@/lib/iron-session/auth/auth.actions";
import {
  ChevronRight,
  LogOut,
  MapPin,
...
export default function ProfileComponent({
  ...
  useEffect(() => {
    if (!authSnapshot?.authenticated) navigate.push("/login");
  }, [authSnapshot?.authenticated, navigate]);

  if (!authSnapshot?.authenticated || !authSnapshot?.user) return null;

  const initials = authSnapshot.user.name
    .split(" ")
    .map((n) => n[0])
    .join("")
    .toUpperCase()
